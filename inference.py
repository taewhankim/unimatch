...
from ..motif_datasets import get_dataloader

# torch.backends.cudnn.enabled = False # This line enables large batch, but the speed is similar

cfg = OmegaConf.load("./config/data_config.yaml")
video_dataloader, _ = get_dataloader(
    cfg.data.train_dataset_type,
    cfg.data.train_dataset_type,
    override_kwargs=cfg.data.get("override_kwargs", {})
)
...
def extract_frames(
    ...
    container = decord.VideoReader(video_data, num_threads=1)
...
def main():
    ...
    ckpt = torch.load("./pretrained_weights/gmflow-scale2-regrefine6-mixdata-train320x576-4e7b215d.pth")
